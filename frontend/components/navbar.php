<nav class="fixed w-full z-50 bg-white/80 backdrop-blur-md border-b border-blue-200/50">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between items-center h-16">
            <div class="flex items-center">
                <div class="flex-shrink-0 flex items-center">
                    <div class="w-10 h-10 bg-gradient-to-br from-blue-500 to-blue-600 rounded-xl flex items-center justify-center mr-3">
                        <i class="fas fa-anchor text-white text-lg"></i>
                    </div>
                    <span class="text-blue-900 font-bold text-xl"><a href="./frontend">Zona Laut</a></span>
                </div>
            </div>
            <div class="hidden md:block">
                <div class="ml-10 flex items-baseline space-x-8">
                    <a href="#" class="text-blue-800 hover:text-blue-600 px-3 py-2 rounded-md text-sm font-medium transition duration-300">Beranda</a>
                    <a href="#" class="text-blue-800 hover:text-blue-600 px-3 py-2 rounded-md text-sm font-medium transition duration-300">Fitur</a>
                    <a href="#" class="text-blue-800 hover:text-blue-600 px-3 py-2 rounded-md text-sm font-medium transition duration-300">Tentang</a>
                    <a href="#" class="text-blue-800 hover:text-blue-600 px-3 py-2 rounded-md text-sm font-medium transition duration-300">Kontak</a>
                </div>
            </div>
            <div>
                <a href="frontend/auth/login/" class="bg-blue-600 hover:bg-blue-700 text-white px-6 py-2.5 rounded-xl font-medium transition duration-300 shadow-md hover:shadow-lg">
                    Dashboard
                </a>
            </div>
        </div>
    </div>
</nav>
